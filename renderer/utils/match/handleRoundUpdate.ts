...
import { MATCHES_DIR, MATCH_FILENAME_EXTENSION, THROWS_PER_ROUND } from 'utils/constants'
...
export const handleRoundUpdate = (
  ...
  const roundTotal = getTotalRoundScore(roundThrows.map((roundThrow) => roundThrow.score))
  ...
  if (
    (matchData.matchCheckout === 'Single' &&
      ...
      !lastThrow.isTriple &&
      scoreLeft === 0) ||
    (matchData.matchCheckout === 'Double' && lastThrow.isDouble && scoreLeft === 0) ||
    (matchData.matchCheckout === 'Triple' && lastThrow.isTriple && scoreLeft === 0) ||
