...
const PreviousMatchesPage: NextPage = () => {
  const { data, isFetching, isLoading, isSuccess } = useMatches();
  const router = useRouter();

  const sortedMatchDataByNewest = data.sort(
  ...
  return (
    ...
    >
      <PageHeader title="Previous Matches">
        {pkg.productName} found {data.length}{" "}
        {data.length === 1 ? "Match" : "Matches"} on your device.
      </PageHeader>
      <Card>
